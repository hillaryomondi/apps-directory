...
@section('title', trans('strathmore/admin-auth::admin.login.title'))
						...
						<auth-form
							...
							<form class="form-horizontal" role="form" method="POST" action="{{ url('/admin/login') }}" novalidate>
								...
								<div class="auth-header">
									<h1 class="auth-title">{{ trans('strathmore/admin-auth::admin.login.title') }}</h1>
									<p class="auth-subtitle">{{ trans('strathmore/admin-auth::admin.login.sign_in_text') }}</p>
								</div>
								<div class="auth-body">
									...
									<div class="form-group" :class="{'has-danger': errors.has('password'), 'has-success': fields.password && fields.password.valid }">
										<label for="password">{{ trans('strathmore/admin-auth::admin.auth_global.password') }}</label>
										<div class="input-group input-group--custom">
											<div class="input-group-addon"><i class="input-icon input-icon--lock"></i></div>
											<input type="password" v-model="form.password"  class="form-control" :class="{'form-control-danger': errors.has('password'), 'form-control-success': fields.password && fields.password.valid}" id="password" name="password" placeholder="{{ trans('strathmore/admin-auth::admin.auth_global.password') }}">
										</div>
									...
									<div class="form-group">
										...
										<button type="submit" class="btn btn-primary btn-block btn-spinner"><i class="fa"></i> {{ trans('strathmore/admin-auth::admin.login.button') }}</button>
									</div>
									<div class="form-group text-center">
										<a href="{{ url('/admin/password-reset') }}" class="auth-ghost-link">{{ trans('strathmore/admin-auth::admin.login.forgot_password') }}</a>
									</div>
								</div>
							</form>
						</auth-form>
