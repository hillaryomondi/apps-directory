...
class RolesController extends Controller
{
    ...
    public function store(StoreRole $request)
    {
        ...
        if ($request->ajax()) {
            return ['redirect' => url('admin/roles'), 'message' => trans('strathmore/admin-ui::admin.operation.succeeded')];
        }
    ...
    public function edit(Role $role)
    {
        ...
        $permissions = $role->permissions_matrix;
    ...
    public function update(UpdateRole $request, Role $role)
    {
        ...
        $permissions_matrix = collect($sanitized)->get('permissions_matrix');

        $sanitized = collect($sanitized)->except('permissions_matrix')->toArray();
        // Update changed values Role
        $role->update($sanitized);
        //Update permissions
        foreach ($permissions_matrix as $group) {
            $perms = json_decode(json_encode($group));
    ...
    {
        abort(403, "Deleting is disabled at the moment");
        $role->delete();
