...
class DepartmentsController extends Controller
{
    ...
    public function index(IndexDepartment $request)
    {
        ...
        return view('frontend.department.index', ['data' => $data]);
    }
    ...
    public function store(StoreDepartment $request)
    {
        ...
        if ($request->ajax()) {
            return ['redirect' => url('departments'), 'message' => trans('strathmore/admin-ui::admin.operation.succeeded')];
        }
        ...
        DB::transaction(static function () use ($request) {
            collect($request->data['ids'])
                ...
                ->each(static function ($bulkChunk) {
                    DB::table('departments')->whereIn('id', $bulkChunk)
                        ->update([
